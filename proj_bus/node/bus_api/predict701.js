...
const { stationId_701 } = require('./route701');
...
async function buspredict() {
  ...
  docClient.get(stateparams, (err, data) => {
    ...
    if (states === '운행 중'){
      for (let i = 0; i < stationId_701.length; i++) {
        try {
          ...
          var queryParams = '?' + encodeURIComponent('serviceKey') + '=' + PUBLIC_API_KEY;
              queryParams += '&' + encodeURIComponent('stationId') + '=' + encodeURIComponent(stationId_701[i]);
